...
- **A failed lazy chunk is recovered from, not reported.** `src/boot/chunk-recovery.ts` (second in `boot`) listens for Vite's `vite:preloadError` and for `router.onError`, and reloads the page once — onto the navigation that was in flight, which it learns from a `router.beforeEach`, since `router.onError` arrives a rejection too late to supply it. Every route component is a lazy `import()`, and a missing `/assets/…` file is not a 404: `firebase.json`'s last rewrite hands it `main.html`, so the browser refuses `text/html` as a module script. That happens to any tab open across a deploy, and to any flaky connection. While the reload is committed `isRecovering()` makes the browser SDK's `beforeSend` drop events — the original rejection *and* the knock-ons from code handed `undefined` where it expected a module. A second failure within the 10-second cooldown is left alone to reach Sentry, because by then reloading is not the answer. Two imports the build leaves unwrapped (`firebase/auth`, `@firebase/functions` — they have no dependencies to preload) never raise that event, so `HeaderSidebar.vue` calls the recovery explicitly on the `init()` every visitor triggers.
...
- **`public/fonts/tw-kai/` is generated, not authored.** Printing asks for a Kai face; readers with 標楷體 installed use theirs, and everyone else (every phone) downloads 全字庫正楷體. The 35 MB source TTF is sliced into 68 unicode-range woff2 subsets by `scripts/build-kai-webfont.py` (fonttools, run by hand — `yarn build` does not do it), ordered so the site's commonest characters land in the first slices. Do not hand-edit `public/fonts/tw-kai.css`; re-run the script. `/fonts/**` is served `immutable`, so a regeneration must change the file names.

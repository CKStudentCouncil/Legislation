...
- **User HTML**: authored via `ProEditor.vue` (wraps Quasar `<q-editor>`), stored as raw HTML, rendered through `SafeHtml.vue` which sanitizes **asynchronously** (`customSanitize` in `utils.ts`, lazy-imports `sanitize-html`) — content is blank on first synchronous render and is never sanitized during SSR.
